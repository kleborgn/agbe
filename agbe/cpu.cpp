...
bool cpu_step() {
	if (!ctx.bHalted)
	{
		...
		printf("%08llu - %04X: %-7s (%02X %02X %02X) A: %02X F: %s BC: %02X%02X DE:%02X%02X HL:%02X%02X\n",
			emu_get_context()->ticks, pc, inst_name(ctx.cur_inst->type), ctx.cur_opcode, bus_read(pc + 1), bus_read(pc + 2), 
	...
	if (ctx.bInterrupt_master_enabled)
	{
