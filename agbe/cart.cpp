...
static const std::map<uint32_t, std::string> LIC_CODE = {
    {0x00, "None"},
    {0x01, "Nintendo R&D1"},
    {0x08, "Capcom"},
...
const char *cart_lic_name()
{
	...
	{
		return LIC_CODE.at(ctx.header->lic_code).c_str();
	}
    return "UNKNOWN";
}
...
bool cart_load(char* cart)
{
    ...
    if (!fp)
    {
        std::cout << "[-] Failed to open: " << cart << std::endl;
        return false;
    }

    std::cout << "[+] Opened: " << ctx.filename << std::endl;
